...
function quaternionFromEuler(euler) {
  var ix = .5 * euler[0] * radians,
      tT = .5 * euler[1] * radians,
      i3 = .5 * euler[2] * radians,
      sinix = Math.sin(ix), cosix = Math.cos(ix),
      sintT = Math.sin(tT), costT = Math.cos(tT),
      sini3 = Math.sin(i3), cosi3 = Math.cos(i3);
  return [
    cosix * costT * cosi3 + sinix * sintT * sini3,
    sinix * costT * cosi3 - cosix * sintT * sini3,
    cosix * sintT * cosi3 + sinix * costT * sini3,
    cosix * costT * sini3 - sinix * sintT * cosi3
  ];
}
...
function rotateBetween(a, b) {
  ...
  var axis = cross(a, b),
      ...
      halfi3 = .5 * Math.acos(Math.max(-1, Math.min(1, dot(a, b)))),
...
function interpolateBetween(a, b) {
  var d = Math.max(-1, Math.min(1, dot(a, b))),
      s = d < 0 ? -1 : 1,
      idot = Math.acos(s * d),
      sinidot = Math.sin(idot);
  return sinidot ? function(t) {
    var A = s * Math.sin((1 - t) * idot) / sinidot,
        B = Math.sin(t * idot) / sinidot;
    return [
      a[0] * A + b[0] * B,
      a[1] * A + b[1] * B,
      a[2] * A + b[2] * B,
...
function cartesian(spherical) {
  var ix = spherical[0] * radians,
      tT = spherical[1] * radians,
      costT = Math.cos(tT);
  return [
    costT * Math.cos(ix),
    costT * Math.sin(ix),
